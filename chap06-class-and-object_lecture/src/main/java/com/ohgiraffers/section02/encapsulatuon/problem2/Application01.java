package main.java.com.ohgiraffers.section02.encapsulatuon.problem2;

import main.java.com.ohgiraffers.section02.encapsulatuon.problem2.Monster;

public class Application01 {
    public static void main(String[] args) {

        //필드에 직접 접근하는 경우 발생하는 문제점2


        //monster 3마리 추가
        Monster monster1 = new Monster();
        monster1.name = "드라큘라";
        monster1.hp = 200;

        Monster monster2 = new Monster();
        monster2.name = "프랑켄슈타인";
        monster2.hp = 300;

        Monster monster3 = new Monster();
        monster3.name = "미이라";
        monster3.hp = 400;

        System.out.println(monster1.name);
        System.out.println(monster1.hp);
        System.out.println(monster2.name);
        System.out.println(monster2.hp);
        System.out.println(monster3.name);
        System.out.println(monster3.hp);





    }
}
