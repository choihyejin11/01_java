...
public class Application01 {
    public static void main(String[] args) {
        ...
        * 객체간의 상호작용을 한다는 사실은 현실 세계와 동일하지만, 의인화 기법이 적용되어
        ...
        * 유연성 확보는 여러곳에 적용될 수 있는 유연한 객체를 의미하며, 즉 재사용성이 높아질 수 있게 한다는 의미이다.
        * 객체의 재사용성이 증가하면 중복 작성되는 코드를 줄일 수 있으며, 오류 발생 가능성을 감소시키고 유지보수성을 증가시킨다.
        *
        * */
        /*
        * 객체와 객체의 상호작용이란?
        * 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메세지(메소드 호출)를 보내는 쪽은 송신자라고 하고,
        ...
        * 따라서 객체를 설계할 때는 속성은 부가적이며, 적절한 책임을 가진 객체들간의 주고 받는 메시지를 이용해 상호작용하는 것을
        * 설계하는 것을 의미한다.
        ...
        * */
        /*
        * 프로그램 개요
        * 카레이서가 자동차를 유전하는 프로그램
        *
        * 시스템 요구사항
        * 1. 카레이서는 시동걸기, 엑셀 밝기, 브레이크 밟기, 시동끄기를 할 수 있다.
        * 2. 자동차는 시동걸기, 앞으로 가기, 멈추기, 시동끄기를 할 수 있다.
        * 3. 자동차는 처음에는 멈춘 상태로 대기 하고 있는다.
        * 4. 카레이서는 먼저 자동차에 시동을 건다. 이미 걸려있는 경우 다시 시동을 걸 수 없다.
        * 5. 카레이서가 엑셀을 밟으면 시동이 걸린 상태일 경우 자동차는 시속이 10km/h 씩 증가하며 앞으로 나아간다.
        * 6. 자동차가 달리는 중인경우 브레이크를 밝으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        * 7. 브레이크를 밟을때 자동차가 달리는 중이 아니라면 이미 멈춰있는 상태를 안내한다.
        ...
        * 2. 객체간 상호작용
        *
        * - 카레이서가 수신 할 수 있는 메세지(카레이서가 해야하는 일)
        *   1. 시동을 걸어라
        *   2. 엑셀을 밟아라
        *   3. 브레이크를 밟아라
        *   4. 시동을 꺼라
        ...
        * 1. 카레이서 클래스
        * - 속성: 자동차
        * - 행위: 시동을 걸어라, 엑셀을 밟아라, 브레이크를 밟아라, 시동을꺼라
        *
        * 2. 자동차 클래스
        * -속성: 시동상태, 현재시속
        ...
        *        * */
        ...
        while (true){
            ...
            switch (no){
                ...
                case 2: carRacer.stapAceletor(); break;
                case 3: carRacer.stopBreak(); break;
                case 4: carRacer.trunoff(); break;
                case 9:
