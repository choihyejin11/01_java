...
//        int bonus = 100;
        ...
        /*
        * 변수에는 자료형이 존재한다.
        * 자료형이란? 다양한 값이 형태별로 어느 정도의 크기를 하나의 값으로 취급할 것인지 미리 컴파일러와 약속한 키워드이다.
        * 메모리를 효율적으로 쓰기 위해 자료형이 생겼다.
        * 예) 앞에서 사용한 int 자료형은 4byte(32비트 / 1byte당 8비트) 만큼을 읽어서 하나의 값으로 취급하겠다는 약속이다.
...
//        문자를 취급하는 자료형은 한가지 이다.
       char ch; //2byte
        ...
        /*논리값을 취급하는 자료형*/
        /* 1byte 논리 데이터 형은 0과 1과의 값으로 데이터를 취급하기 때문에 1bit로 표현 가능하지만, 최소단위가 1byte이기 때문에 1byte
        * 크기를 갖는다
         */
        ...
        * 변수를 대입연산자 왼쪽에 사용하면 공간의 의미이고, 오른쪽에 사용하면 변수가 가진 값을 의미한다.
        ...
        snum = 2;
        inum = 4;
        lnum = 8L ; //뒤에 l을 꼭 넣어주어야 한다.
        System.out.println(lnum);

        /*  실수를 취급하는 자료형에 값 대입 */
        fnum = 4.0f; //뒤에 f를 써줘야 한다.
//        fnum = 4.0 ; 이거는 에러됨
        dnum = 4.0;
        ...
        * 정수는 int, 실수는 double 이다.
        ...
        System.out.println("보너스와 포인트의 합은:" + (bonus + point));

        /* 대입연산자의 왼쪽과 오른쪽 편에 사용하기 */
        bonus = bonus + 100; //bouns +=100; 과 같은 의미
